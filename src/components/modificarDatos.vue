...
export default {
  ...
  methods: {
    async fetchUserData(userId) {
      try {
        const response = await axios.get(`${import.meta.env.PUBLIC_URL}/api/user/${userId}`);
    ...
    async fetchRoles() {
      try {
        const response = await axios.get(`${import.meta.env.PUBLIC_URL}/api/roles`); 
    ...
    async submitForm() {
      const userId = this.$route.params.id;
      try {
        await axios.patch(`${import.meta.env.PUBLIC_URL}/api/user/${userId}`, {
          ...this.user,
